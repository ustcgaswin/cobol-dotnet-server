...
def create_documentation_graph(llm, analyzer, tools_dict):
    ...
    Updated Flow: 
    Researcher -> (Tools loop) -> Unified Generation -> End
    
    Note: We no longer branch for Technical vs Functional generation here 
    because the new prompts extract BOTH sets of data in a single LLM call.
    """
    nodes = DocAgentNodes(llm, analyzer, tools_dict)
    workflow = StateGraph(DocAgentState)
    ...
    # We use 'technical_gen_node' as the Unified Extractor (it returns both json keys)
    workflow.add_node("unified_generator", nodes.technical_gen_node)
    
    # This node is now a pass-through/state collector
    workflow.add_node("collector", nodes.render_node)
    ...
    # 3. Define Logic: Researcher -> (Tools OR Generation)
    def route_research(state: DocAgentState):
        """Decides if we continue researching or start generating data."""
    ...
    # 5. Generation -> Collector -> End
    # No conditional logic needed here anymore; extraction is unified.
    workflow.add_edge("unified_generator", "collector")
