        ...
        try:
            ...
            for line in content.splitlines():
                if line.startswith("# File:"):
                    # Extract filename from "# File: path/to/file.cbl"
                    fname = line.replace("# File:", "").strip()
                    # Keep just the basename for simpler tracking if preferred, 
                    # but relative path is safer. Ideally matches what's used in the catalog.
            ...
            logger.error(f"initialize_file_tracker error: {e}")
            return f"Error initializing tracker: {e}"
